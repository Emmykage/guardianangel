...
import { AddAssignmentButton, AddAssignmentForm, AddAssignmentInput, AddAssignmentTextArea, AssignmentItem, AssignmentList, AssignmentsContainer, AssignmentsContent, AssignmentsHeader, Content } from '../../styles/AssignmentsStyles'
...
const Assignment = () => {
  ...
  const handleAddAssignment = async (e) => {
    e.preventDefault()
    if(newAssignment.title.trim() !== "" && newAssignment.description.trim() !== "" && newAssignment.grade.trim() !== ""){
      try {
        const response = await axios.post('http://localhost:4000/api/v1/students', newAssignment);
        console.log("Response data", response.data)
        setAssignments([...assignments, response.data.assignment] )
        setNewAssignment ({title: "", description: "", subject: '', grade: ""})
      } catch (error) {
